...
class SPECTReconLogic(ScriptedLoadableModuleLogic):
    ...
    def get_system_matrix(self, file_NM, attenuation_toggle, ct_file, psf_toggle, collimator, intrinsic_resolution, index_peak, advanced_collimator_code):
        ...
        if psf_toggle:
            if advanced_collimator_code is not None:
                psf_operator = get_psf_operator(advanced_collimator_code)
        ...
        # Cutoff transform if using large PSF to avoid edge artifacts
        if advanced_collimator_code is not None:
            mask_cutoff = dicom.get_FOV_mask_from_projections(file_NM)
            cutoff_transform = CutOffTransform(mask_cutoff)
            obj2obj_transforms.append(cutoff_transform)
